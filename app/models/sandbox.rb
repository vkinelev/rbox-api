 class Sandbox < ApplicationRecord
  ...
  def install_git_post_receive_hook
    dest_hooks_dir = File.join(path_to_git_repository, 'hooks')
    FileUtils.cp(Rails.root.join('post-receive'), dest_hooks_dir)
  end
  ...
  def git_public_repository_url
    [ENV['GIT_PUBLIC_BASE_URL'], 'git', git_repository_basename].join('/')
  end
  ...
  def app_url
    'http://' + ENV['BASE_HOST_FOR_SANDBOXES'] + ":#{app_port}"
  end

  def docker_service_options(image_name)
